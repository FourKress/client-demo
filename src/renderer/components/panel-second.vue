<template>
  <div class="tab-panel">
    <el-form
      class="form-panel"
      ref="form"
      :model="form"
      :rules="rules"
      label-width="150px"
    >
      <el-form-item label="boundary_files" prop="boundary_files">
        <el-button type="primary" @click="onSelect('boundary_files')">
          选择
        </el-button>
        <div class="tag-panel" v-if="form.boundary_files.length">
          <el-tag
            v-for="path in form.boundary_files"
            :key="path"
            closable
            @close="handlePathClose(path, 'boundary_files')"
          >
            {{ path }}
          </el-tag>
        </div>
      </el-form-item>

      <el-form-item
        label="wind_files"
        prop="wind_files_item"
        :rules="[
          {
            required: !windList.length,
            message: '请选择',
            trigger: ['blur', 'change'],
          },
          {
            validator: validateFnc,
            trigger: ['blur', 'change'],
          },
        ]"
      >
        <el-button type="primary" @click="onSelectOnly('wind_files_item')">
          选择
        </el-button>
        <el-tag
          v-if="form.wind_files_item"
          closable
          @close="handlePathCloseOnly('wind_files_item')"
        >
          {{ form.wind_files_item }}
        </el-tag>
      </el-form-item>

      <div class="form-row">
        <el-form-item
          style="width: 360px"
          label="wind_direction"
          prop="wind_direction_item"
          :rules="[
            {
              required: !windList.length,
              message: '请输入',
              trigger: ['blur', 'change'],
            },
            {
              validator: validateFnc,
              trigger: ['blur', 'change'],
            },
          ]"
        >
          <el-input v-model="form.wind_direction_item"></el-input>
          <div class="tag-panel" style="width: 630px" v-if="windList.length">
            <el-tag
              v-for="(wind, index) in windList"
              :key="wind.path"
              closable
              @close="handleWindPathClose(index)"
            >
              {{ wind.path }} / {{ wind.direction }}
            </el-tag>
          </div>
        </el-form-item>
        <div class="row-right">
          <el-button
            style="width: 60px; margin-left: 24px"
            type="primary"
            @click="onAdd()"
          >
            添加
          </el-button>
        </div>
      </div>

      <el-form-item label="dir_ground_file" prop="dir_ground_file">
        <el-button type="primary" @click="onSelectOnly('dir_ground_file')">
          选择
        </el-button>
        <el-tag
          v-if="form.dir_ground_file"
          closable
          @close="handlePathCloseOnly('dir_ground_file')"
        >
          {{ form.dir_ground_file }}
        </el-tag>
      </el-form-item>

      <el-form-item label="dir_mesh_file" prop="dir_mesh_file">
        <el-button type="primary" @click="onSelectOnly('dir_mesh_file')">
          选择
        </el-button>
        <el-tag
          v-if="form.dir_mesh_file"
          closable
          @close="handlePathCloseOnly('dir_mesh_file')"
        >
          {{ form.dir_mesh_file }}
        </el-tag>
      </el-form-item>

      <el-form-item label="dir_measured_wind" prop="dir_measured_wind">
        <el-button type="primary" @click="onSelectOnly('dir_measured_wind')">
          选择
        </el-button>
        <el-tag
          v-if="form.dir_measured_wind"
          closable
          @close="handlePathCloseOnly('dir_measured_wind')"
        >
          {{ form.dir_measured_wind }}
        </el-tag>
      </el-form-item>
    </el-form>
  </div>
</template>

<script>
import Mixins from '../mixins';

export default {
  name: 'PanelSecond',
  mixins: [Mixins],
  data() {
    return {
      windList: [],
      form: {
        boundary_files: [],
        wind_files: '',
        wind_direction: '',
        wind_files_item: '',
        wind_direction_item: '',

        dir_ground_file: '',
        dir_mesh_file: '',
        dir_measured_wind: '',
      },
      rules: {
        boundary_files: [
          { required: true, message: '请选择', trigger: ['blur', 'change'] },
        ],
        wind_files: [
          { required: true, message: '请选择', trigger: ['blur', 'change'] },
        ],
        wind_direction: [
          { required: true, message: '请输入', trigger: ['blur', 'change'] },
        ],
        wind_files_item: [
          { required: true, message: '请选择', trigger: ['blur', 'change'] },
        ],
        wind_direction_item: [
          { required: true, message: '请输入', trigger: ['blur', 'change'] },
        ],

        dir_ground_file: [
          { required: true, message: '请选择', trigger: ['blur', 'change'] },
        ],
        dir_mesh_file: [
          { required: true, message: '请选择', trigger: ['blur', 'change'] },
        ],
        dir_measured_wind: [
          { required: true, message: '请选择', trigger: ['blur', 'change'] },
        ],
      },
    };
  },
  methods: {
    onAdd() {
      this.$refs.form.validateField('wind_files_item');
      this.$refs.form.validateField('wind_direction_item');
      const { wind_direction_item, wind_files_item } = this.form;
      if (wind_direction_item && wind_files_item) {
        this.windList.push({
          direction: wind_direction_item,
          path: wind_files_item,
        });
        this.$refs.form.validateField('wind_direction_item');
      }
    },

    handleWindPathClose(index) {
      this.windList.splice(index, 1);
      this.$refs.form.validateField('wind_direction_item');
    },

    validateFnc(rule, value, callback) {
      if (!this.windList?.length) {
        if (rule.field === 'wind_files_item' && !this.form.wind_direction_item) {
          this.$refs.form.validateField('wind_direction_item');
        }
        if (rule.field === 'wind_direction_item') {
          if (!this.form.wind_files_item) {
            this.$refs.form.validateField('wind_files_item');
          } else {
            callback('请点击右侧添加按钮');
          }
        }
      }
      callback();
    },

    validate() {
      return new Promise((resolve) => {
        this.$refs.form.validate((valid) => {
          resolve(valid);
        });
      });
    },
  },
};
</script>

<style scoped lang="less">
.tab-panel {
  height: 100%;
  display: flex;
  flex-direction: column;
}

.form-panel {
  flex: 1;
  width: 780px;
  .tag-panel {
    width: 100%;
    max-height: 150px;
    overflow-y: auto;
    overflow-x: hidden;
    padding: 6px 12px;
    border: 1px solid #ccc;
    margin-top: 8px;
    box-sizing: border-box;
  }

  .el-tag {
    display: flex;
    align-items: center;
    justify-content: space-between;
    margin: 6px 0;
  }

  .form-row {
    display: flex;
    justify-content: flex-start;
    align-items: flex-start;
  }
}
</style>
